...
class ElderTracker(config: ApplicationConfig, dbConnection: DbConnection) extends Actor {
  ...
  private def logOmni(): Unit = {
    ...
    dbConnection.logElderStatus(elderData)
  }

  private def fetchElderData(): Seq[ElderInfo] = {
